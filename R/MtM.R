...
#'
#' Carries out model-based clustering using a multivariate t mixture (MtM). The
#' function will determine itself if the data set is complete or incomplete and
#' fit the appropriate model accordingly. When using this function, the data set
#' must be at least bivariate, and missing values are assumed to be missing at
#' random (MAR).
#'
...
MtM <- function(
  ...
  if(is.null(ncol(X)) | ncol(X) < 2) {
    stop("X must be at least bivariate. For a univariate data set, please use function tM.")
  }
  ...
  if (any(is.na(X))) {
    mod <- MtM_incomplete_data(X, G, max_iter, epsilon, init_method, equal_prop,
                               identity_cov, df0, outlier_cutoff, show_progress, manual_clusters)
  } else {
    mod <- MtM_complete_data(X, G, max_iter, epsilon, init_method, equal_prop,
                             identity_cov, df0, outlier_cutoff, show_progress, manual_clusters)
  }
...
MtM_incomplete_data <- function(
  ...
  while(iter < max_iter & getall(ll) > epsilon) {
    ...
    for (g in 1:G) {
      for (i in 1:n) {
        ...
        z[i, g] <- mvtnorm::dmvt(xi[o], delta = mu[g, o], sigma = sigma_oo,
                                 df = df[g], log = FALSE)
        w_tilde[i, g] <- (df[g] + sum(o)) / (df[g] + mahalanobis(xi[o], mu[g, o], sigma[o, o, g]))
      }
    }
    ...
    for (g in 1:G) {
      ...
      root <- uniroot.all(function(a) {
        ...
        C <-  sum(z_tilde[, g] * (digamma((df[g] + do) / 2) - log((df[g] + do) / 2)))
        C <-  C / N[g]

        return(A + B + C)
      }, lower = 0.001, upper = 200)
    ...
    for (g in 1:G) {
      for (i in 1:n) {
        ...
        dens[i, g] <- mvtnorm::dmvt(xi[o], delta = mu[g, o], sigma = sigma_oo,
                                    df = df[g], log = FALSE)
      }
    }
  ...
  outliers <- sapply(1:n, function(i) {
    delta <- mahalanobis(X_imputed[i, ], center = mu[clusters[i], ],
                         cov = sigma[, , clusters[i]])
    1 - pchisq(delta, df = d) < 1 - outlier_cutoff
  })
...
MtM_complete_data <- function(
  ...
  while(iter < max_iter & getall(ll) > epsilon) {
    ...
    for (g in 1:G) {
      z[, g] <- mvtnorm::dmvt(X, delta = mu[g, ], sigma = sigma[, , g], df = df[g], log = FALSE)
      w_tilde[, g] <- (df[g] + d) / (df[g] + mahalanobis(X, mu[g, ], sigma[, , g]))
    ...
    dens <- matrix(NA, nrow = n, ncol = G)
    for (g in 1:G) {
      dens[, g] <- mvtnorm::dmvt(X, delta = mu[g, ], sigma = sigma[, , g], df = df[g], log = FALSE)
    }
  ...
  outliers <- sapply(1:n, function(i) {
    delta <- mahalanobis(X[i, ], center = mu[clusters[i], ], cov = sigma[, , clusters[i]])
    1 - pchisq(delta, df = d) < 1 - outlier_cutoff
  })
