  ...
  while (iter < max_iter & getall(loglik) > epsilon) {
    ...
    for (g in 1:G) {
      for (j in 1:np) {
        ...
        dens[Im[[j]], g] <- mvtnorm::dmvt(Xo_j, delta = mu_o, sigma = as.matrix(Sigma_oo), df = 1, log = FALSE)
...
###########################################################################
###                                                                     ###
###            Multivariate Cauchy Mixture for Complete Data            ###
###                                                                     ###
###########################################################################

MCM_complete_data <- function(
    X,
    G,
    max_iter       = 20,
    epsilon        = 0.01,
  ...
  while (iter < max_iter & getall(loglik) > epsilon) {
    ...
    for (g in 1:G) {
      dens[, g] <- mvtnorm::dmvt(X, delta = mu[g, ], sigma = as.matrix(Sigma[, , g]), df = 1, log = FALSE)
    }
