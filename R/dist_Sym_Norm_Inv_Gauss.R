####################################################################################
###                                                                              ###
###  Multivariate Symmetric Normal-Inverse Gaussian Mixture for Incomplete Data  ###
###                                                                              ###
####################################################################################

MSNIGM_incomplete_data <- function(
    X,
    G,
    max_iter     = 20,
    epsilon      = 0.01,
...
##################################################################################
###                                                                            ###
###  Multivariate Symmetric Normal-Inverse Gaussian Mixture for Complete Data  ###
###                                                                            ###
##################################################################################

MSNIGM_complete_data <- function(
    X,
    G,
    max_iter     = 20,
    epsilon      = 0.01,
  ...
  while (iter < max_iter & getall(loglik) > epsilon) {
    ...
    for (g in 1:G) {
      dens[, g] <- dGH(X, mu = mu[g, ], Sigma = Sigma[, , g], beta = beta[g, ],
                       lambda = lambda[g], omega = omega[g])
    }
